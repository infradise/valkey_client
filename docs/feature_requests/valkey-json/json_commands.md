## JSON Commands

### Check List

* **JSON Data Format**: JSON-based document formats for storing structured payloads.
    * Redis JSON and Valkey JSON (aka. valkey-json)
    * [x] jsonArrAppend,  [ ] jsonArrAppendEnhanced
    * [x] jsonArrIndex,   [ ] jsonArrIndexEnhanced
    * [x] jsonArrInsert,  [ ] jsonArrInsertEnhanced
    * [x] jsonArrLen,     [ ] jsonArrLenEnhanced
    * [x] jsonArrPop,     [ ] jsonArrPopEnhanced
    * [x] jsonArrTrim,    [ ] jsonArrTrimEnhanced
    * [ ] jsonClear
    * [x] jsonDel
    * [ ] jsonForget
    * [x] jsonGet
    * [x] jsonMerge
    * [ ] jsonMget
    * [ ] jsonMset
    * [ ] jsonNumincrby
    * [ ] jsonNummultby
    * [ ] jsonObjkeys,    [ ] jsonObjkeysEnhanced
    * [x] jsonSet
    * [ ] jsonStrappend,  [ ] jsonStrappendEnhanced
    * [ ] jsonStrlen,     [ ] jsonStrlenEnhanced

### Roadmap

* **v2.5.5**: `jsonObjkeysEnhanced`, `jsonStrappendEnhanced`, `jsonStrlenEnhanced`
* **v2.5.4**: `jsonArrAppendEnhanced`, `jsonArrIndexEnhanced`, `jsonArrInsertEnhanced`, `jsonArrLenEnhanced`, `jsonArrPopEnhanced`,`jsonArrTrimEnhanced`
* **v2.5.3**: `jsonObjkeys`, `jsonStrappend`, `jsonStrlen`
* **v2.5.2**: `jsonClear`, `jsonForget`, `jsonMget`, `jsonMset`, `jsonNumincrby`,`jsonNummultby`
* **v2.5.1**: `jsonArrAppend`, `jsonArrIndex`, `jsonArrInsert`, `jsonArrLen`, `jsonArrPop`,`jsonArrTrim`
* **v2.5.0**: `jsonSet`, `jsonDel`, `jsonGet`, `jsonMerge`